...
\]
Der Cosinus-Term faellt dabei weg, weil bei der Produktregel zwei Cosinus-Anteile mit
entgegengesetztem Vorzeichen entstehen: Schreibe $x_a=1-\mathrm{e}^{-a t}\!\left(\cos(bt)+\tfrac{a}{b}\sin(bt)\right)$
mit $a=D\omega_0$, $b=\omega_d$, dann gilt $\dot x_a=\mathrm{e}^{-a t}\bigl(a f - f'\bigr)$.
Dabei heben sich die $a\cos(bt)$-Terme aus $a f$ und $f'$ exakt auf, so dass nur ein Sinus-Term bleibt.
